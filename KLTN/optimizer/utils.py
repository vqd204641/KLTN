...
def get_fuzzy_charging_time(network=None, mc = None, q_learning=None, time_stem=0, state=None):
    ...
    E_min_crisp = network.listNodes[network.min_node()].energy * 10 / network.listNodes[0].warning
    ...
    L_r['H'] = fuzz.trapmf(L_r.universe, [6, 10, nb_of_nodes , nb_of_nodes])
    ...
    # energy_min = network.listNodes[0].threshold + alpha * network.listNodes[0].energy_max
    energy_min = network.listNodes[0].threshold + alpha * (network.listNodes[0].capacity - network.listNodes[0].threshold)
    s1 = []  # list of node in request list which has positive charge
    s2 = []  # list of node not in request list which has negative charge
    for node in network.listNodes:
        d = distance.euclidean(q_learning.action_list[state], node.location)
